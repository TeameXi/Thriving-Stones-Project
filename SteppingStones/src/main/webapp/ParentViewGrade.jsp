                    ...
                    for (int child_id: childIDs){
                        ...
                        for (Class tempCls: tempClass){
                            ...
                            for (Class currCls: classes){
                                if (tempCls.getClassID() == currCls.getClassID())
                                    exists = true;
                            }
                            if (!exists){
                                classes.add(tempCls);
...
//                        '<td>' + CA1_0_arr + '</td>';
        ...
        function format(callback, data) {
            var student_ids = "<%=childIDChain%>";
            $.ajax({
                url: 'ParentViewGradeServlet',
                data: {class_id: data["ID"],student_id:student_ids,branch_id: <%=branch_id%>},
                dataType: "json",
